...
class App {
  constructor(config) {
    this.coverageDir = './.coverage_env';          // Env that instrumented .sols are tested in
    ...
    this.tmpdir = config.tmpdir || './.solidity_coverage';
    this.allEventsFile = `${this.tmpdir}/${(config.allEventsFile || '.all_events')}`;
    this.sourceTopicsFile = `${this.tmpdir}/${(config.sourceTopicsFile || '.source_topics')}`;
  ...
  */
    ...
    try {
      shell.ls(`${this.coverageDir}/contracts/**/*.sol`).forEach(file => {
        ...
        if (!this.skipFiles.includes(file) && !this.inSkippedFolder(file)) {
          ...
          const canonicalPath = contractPath.split(this.coverageDir.split("/")[1]).join(working);
          ...
          this.coverage.addContract(instrumentedContractInfo, canonicalPath, this.sourceTopicsFile);
  ...
  generateReport() {
    ...
    return new Promise((resolve, reject) => {
      ...
      const stream = fs.createReadStream(this.allEventsFile);
  ...
  sanityCheckContext(){
    ...
    if (shell.test('-e', `${this.workingDir}/${this.sourceTopicsFile}`)){
      shell.rm(`${this.workingDir}/${this.sourceTopicsFile}`);
    }
  }
  ...
  cleanUp(err) {
    ...
    shell.rm(this.allEventsFile);
    shell.rm(this.sourceTopicsFile);
